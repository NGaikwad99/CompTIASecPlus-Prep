...
| ACL | Access Control List | Rules that permit or deny access to/from a specific IP address, protocols, etc. |
| AES | Advanced Encryption Standard | Secure symmetric block cipher/algorithm. |
| AES-256 | Advanced Encryption Standards 256-bit | AES with a key size of 256 bits. |
| AH | Authentication Header | Provides authentication and integrity for IP packets in IPSec. |
| AI | Artificial Intelligence | Mimicking human thinking, learning, and reasoning using computational systems. |
| AIS | Automated Indicator Sharing | A program by the Cybersecurity and Infrastructure Security Agency (CISA) that facilitates the real-time exchange of machine-readable cyber threat indicators and defensive measures between public and private sector organizations. |
| ALE | Annualized Loss Expectancy | The expected monetary loss due to a risk over a year. Calculated as ALE = SLE * ARO. |
| AP | Access Point | A device that enables wireless connection to wired networks, such as a router. |
| API | Application Programming Interface | A set of rules that facilitates communication between software components, allowing data and functionality sharing. |
| APT | Advanced Persistent Threat | A prolonged and targeted cyberattack where an attacker gains unauthorized access and remains undetected for an extended period. |
| ARO | Annualized Rate of Occurrence | The expected frequency of a risk or event occurring within a year. |
| ARP | Address Resolution Protocol | A network protocol used to resolve MAC addresses from corresponding IP addresses in a local network. |
| ASLR | Address Space Layout Randomization | A security technique that randomizes memory locations to make it harder to exploit memory injection vulnerabilities like buffer overflows. |
| ATT&CK | Adversarial Tactics, Techniques, and Common Knowledge | A public repository developed by MITRE that describes various cyberattacks, including techniques and methods used by attackers. |
| AUP | Acceptable Use Policy | A document stipulating rules of behaviour for users of computers, networks, and associated resources. |
| AV | Antivirus | Software designed to detect, block, and remove viruses and other malicious software. |
| BASH | Bourne Again Shell | A scripting language used in Linux, MacOS, and other UNIX-like operating systems. |
| BCP | Business Continuity Planning | The process of creating systems of prevention and recovery to deal with potential threats to a company, ensuring no downtime during disruptions such as cyberattacks or natural disasters. Also known as Continuity of Operations Planning (COOP). |
| BGP | Border Gateway Protocol | A protocol that facilitates data exchange and communication between autonomous systems over the Internet. |
| BIA | Business Impact Analysis | A process that identifies critical business functions and the impact of a disruption on them. |
| BIOS | Basic Input/Output System | Firmware that performs initial hardware checks after a computer is powered on. |
| BPA | Business Partners Agreement | A formal contract between organizations outlining responsibilities and expectations of each party in the relationship. |
| BPDU | Bridge Protocol Data Unit | A message used by the Spanning Tree Protocol (STP) to prevent network loops and ensure there is only a single active path to each device. |
| BYOD | Bring Your Own Device | A policy that allows employees to use their own devices for work purposes. |
| CA | Certificate Authority | An entity that issues, manages, and revokes digital certificates used in PKIs. |
| CAPTCHA | Completely Automated Public Turing Test to Tell Computers and Humans Apart | A user identification mechanism used as a countermeasure against automated software (such as bots). |
| CAR | Corrective Action Report | A document outlining steps to fix an issue, such as retraining employees after a phishing attack. |
| CASB | Cloud Access Security Broker | An intermediary between an organization's infrastructure and a cloud provider's infrastructure to ensure visibility, data security, compliance, and threat protection. |
| CBC | Cipher Block Chaining | A cryptographic block encryption method where each plaintext block is XORed with the previous ciphertext block before encryption. The first plaintext block is XORed with an initialization vector (IV). |
| CCMP | Counter Mode/CBC-MAC Protocol | An encryption protocol used in wireless LANs (Wi-Fi) communication, such as WPA2, to ensure confidentiality and integrity. |
| CCTV | Closed-circuit Television | A video surveillance system that facilitates real-time monitoring of an area by displaying the footage on a limited set of monitors instead of broadcasting it. |
| CERT | Computer Emergency Response Team | A team dedicated to identifying, analyzing, and countering cyberattacks such as data breaches or denial-of-service attacks. |
...
| PII | Personally Identifiable Information | any type of information about an individual that can be used to uniquely identify that person |
...
| SaaS | Software as a Service | Cloud-based delivery model wherein a cloud service provider develops and maintains a software product being leveraged by their customer, providing regular patches and updates, and charging their client on a pay-as-you-go basis. |
...
| SPF | Sender Policy Framework | Protocol used to authenticate the sender of an email to ensure the legitimacy of the email address. |
